...
import { employmentSchema } from "@/src/lib/intake-contracts/schemas";
import {
  findPublicIntakeByToken,
  updatePublicIntakeEmployment,
...
export async function PATCH(
  ...
  const parsed = await parseJson(request, employmentSchema);
  if (!parsed.success) {
    return parsed.response;
  }

  if (shouldUseMockData()) {
    const saved = saveMockEmployment(token, parsed.data);
    if (!saved) {
  ...
  const updated = await updatePublicIntakeEmployment(intake.id, parsed.data);
  const meta = getRequestMeta(request);
  await createEmployeeAuditLog({
    employeeIntakeId: intake.id,
    action: "saved",
    actionTarget: "employment",
