...
import { attributesSchema } from "@/src/lib/intake-contracts/schemas";
import {
  findPublicIntakeByToken,
  updatePublicIntakeAttributes,
...
export async function PATCH(
  ...
  const parsed = await parseJson(request, attributesSchema);
  if (!parsed.success) {
    return parsed.response;
  }

  if (shouldUseMockData()) {
    const saved = saveMockAttributes(token, parsed.data);
    if (!saved) {
  ...
  const updated = await updatePublicIntakeAttributes(intake.id, parsed.data);
  const meta = getRequestMeta(request);
  await createEmployeeAuditLog({
    employeeIntakeId: intake.id,
    action: "saved",
    actionTarget: "attributes",
